...
if (!isLoggedIn()) {
  ...
  $results = mysqli_query($conn, "SELECT * FROM music");
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <?php foreach ($users as $user): ?>
              <tr>
                <td> <img src="<?php echo 'cover/' . $user['album_cover']; ?>" width="90" height="90" alt=""> </td>
                <td> <?php echo $user['title']; ?> </td>
                <td> <?php echo $user['artist']; ?> </td>
                <td> <?php echo $user['year']; ?> </td>
                <td> <a href="comment-write.php?cover=<?php echo $user['album_cover']; ?>&title=<?php echo $user['title'];?>&artist=<?php echo $user['artist'];?>" onclick="win_memo(this.href); return false;">
                  <button type="button" class="btn btn-sm btn-primary pull-right app-new-msg js-newMsg">코멘트 작성하기</button></a></td>
              </tr>
            <?php endforeach; ?>
          </tbody>
        </table>
